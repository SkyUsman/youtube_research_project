...
'''
Comment filter, based on likes, NaN, comment length, and LLM Classification with Gemini.
The model can be editied within the function.
Edit the path the batch sizes.
...
class ProcessComments:
    ...
    def _parse_comment_block(self, block: str) -> Tuple[str, str, int, str, str]:
        ...
        # Extract information from each line (could either be a comment or reply prefix) - uncomment the 2 lines to include replies..
        for line in lines:
                ...
                with open(file_path, 'r', encoding='utf-8') as file:
                    ...
                    for block in blocks:
                        ...
                        if block:                            
                            ...
                            if (author and comment and self._is_english(comment)
                                and likes > 150 and 20 <= len(comment) <= 300):
...
def main():
    ...
    folder_path = 'china_data_security'
    ...
    output_csv = "china_data_security_filtered.csv"
